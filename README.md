...
## النشر التلقائي لـ APK وAAB

يعمل Workflow `Medacal Android Production` تلقائيًا عند كل `push` إلى الفرع `main`. يبني التطبيق من الصفر، ثم يرفع `Medacal-debug.apk` القابل للتثبيت و`Medacal-production.aab` إلى GitHub Release جديد بالوسم `android-v<run_number>`. يظل التشغيل اليدوي متاحًا من تبويب Actions عبر `workflow_dispatch`، ويكون نشر Release مفعّلًا افتراضيًا.
