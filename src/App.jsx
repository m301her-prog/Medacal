...
const links=[
  ...
  {to:'/invoices',icon:FileText,label:'الفواتير',group:'إدارة الصيدلية'},
...
export default function App(){
  ...
  return <div className="app-shell">
    ...
    <div className="main-area"><header className="topbar"><button className="menu-button" onClick={()=>setOpen(!open)}><Menu size={21}/></button><div className="breadcrumb"><PackageSearch size={17}/> <span>فرما تيك</span><ChevronLeft size={14}/><b>{current?.label|| (location.pathname.startsWith('/invoices')?'الفواتير':location.pathname==='/install'?'تثبيت التطبيق':'الإعدادات')}</b></div><div className="top-actions"><div className="live"><i/> مباشر</div><div className="avatar">س</div></div></header><Routes><Route path="/" element={<Dashboard/>}/><Route path="/chat" element={<CustomerChat/>}/><Route path="/inventory" element={<Inventory/>}/><Route path="/pos" element={<POS/>}/><Route path="/invoices" element={<Invoices/>}/><Route path="/invoices/:number" element={<Invoices/>}/><Route path="/customers" element={<Customers/>}/><Route path="/suppliers" element={<Suppliers/>}/><Route path="/reports" element={<Reports/>}/><Route path="/notifications" element={<Notifications/>}/><Route path="/settings" element={<SettingsPage/>}/><Route path="/install" element={<Install/>}/><Route path="*" element={<Dashboard/>}/></Routes></div>
  </div>
}
