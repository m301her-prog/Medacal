...
const service={
 ...
 async listMedicines(query='',options){try{const data=await request(`${API_ENDPOINTS.medicines}${query?`?q=${encodeURIComponent(query)}`:''}`,{method:'GET',...options});if(!query)OfflineStore.saveMedicines(data.items||[]);return data}catch(error){const items=OfflineStore.getMedicines().filter(x=>!query||JSON.stringify(x).toLowerCase().includes(query.toLowerCase()));if(items.length||isOfflineError(error))return{items,offline:true};throw error}},
 async createMedicine(medicine,options){try{const data=await request(API_ENDPOINTS.medicines,{method:'POST',body:medicine,...options});const items=OfflineStore.getMedicines();OfflineStore.saveMedicines([data.item||medicine,...items]);return data}catch(error){if(!isOfflineError(error))throw error;const item={...medicine,id:medicine.id||`offline-${Date.now()}`,offline:true};OfflineStore.saveMedicines([item,...OfflineStore.getMedicines()]);OfflineStore.enqueue({method:'POST',resource:'medicines',body:medicine});return{item,offline:true}}},
 async removeMedicine(id,options){try{const data=await request(API_ENDPOINTS.medicines,{method:'DELETE',body:{id},...options});OfflineStore.saveMedicines(OfflineStore.getMedicines().filter(x=>x.id!==id));return data}catch(error){if(!isOfflineError(error))throw error;OfflineStore.saveMedicines(OfflineStore.getMedicines().filter(x=>x.id!==id));OfflineStore.enqueue({method:'DELETE',resource:'medicines',body:{id}});return{offline:true}}},
 ...
 async createSale(sale,options){try{return await request(API_ENDPOINTS.sales,{method:'POST',body:sale,...options})}catch(error){if(!isOfflineError(error))throw error;OfflineStore.enqueue({method:'POST',resource:'sales',body:sale});return localSale(sale)}},
 ...
 async syncOfflineQueue(){const queue=OfflineStore.getQueue();if(!queue.length||!navigator.onLine)return{synced:0,pending:queue.length};const pending=[];let synced=0;for(const operation of queue){try{await request(API_ENDPOINTS[operation.resource],{method:operation.method,body:operation.body});synced++}catch(error){pending.push(operation);if(isOfflineError(error))break}}OfflineStore.replaceQueue(pending);if(synced)OfflineStore.markSynced();return{synced,pending:pending.length}},
};
